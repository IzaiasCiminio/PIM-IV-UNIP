...
void main(){
    ...
    if(logado==1){
        ...
        printf("=DATA DE CRIAÇÃO DA EMPRESA=\n");
        printf("DIA:");
        fflush(stdin);
        scanf("%d", &diaN);
        printf("MES:");
        ...
        printf("E-MAIL(CASO NÃO TENHA, COLOCAR N/P): ");
        fflush(stdin);
        gets(email);;
        ...
        fprintf(ponteiroCadastro, "\nNOME RESPONSÁVEL:%s", nomeResponsavel);
        fprintf(ponteiroCadastro, "\nRAZÃO SOCIAL:%s", razaoSocial);
        fprintf(ponteiroCadastro, "\nCNPJ EMPRESA:%s", cnpj);
        fprintf(ponteiroCadastro, "\nDATA DE CRIAÇÃO DA EMPRESA:%d/%d/%d", diaN,mesN,anoN);
